...
def login():
    ...
    users = {"1": "Aleeya", "2": "Izatul", "3": "Faseha", "4": "Ooi Sue Ann"}
...
with st.sidebar:
    ...
    st_text_size = st.slider("Saiz Bearing/Jarak", 5, 30, 12)
    ...
    try:
        ...
        if 'E' in df.columns and 'N' in df.columns:
            ...
            if len(coords) < 3:
                ...
                col_m3.metric("Bilangan Stesen", num_points)

                # 7. PRA-KIRA DATA SEMPADAN (Bearing & Jarak)
                line_data = [] 
                for i in range(num_points):
                    ...
                    line_data.append({'bearing': dd_to_dms(bearing_val), 'dist': dist})

                # 8. GEOSATELIT INTERAKTIF (LOKASI SEBENAR)
                st.divider()
                st.markdown("### 🌍 Pandangan Geosatelit Interaktif")
                try:
                    # Penukaran Koordinat ke GPS (Lat/Lon)
                    ...
                    folium.Polygon(
                        ...
                        color=poly_color, weight=st_line_weight, fill=True, fill_color=poly_color, fill_opacity=0.4,
                        popup=folium.Popup(f"<b>Info Lot</b><hr>Luas: {luas:,.2f} m²<br>Parameter: {perimeter:,.2f} m", max_width=250),
                        tooltip="Klik untuk Info Lot"
                    ).add_to(m)

                    # B. PAPARAN BEARING & JARAK PADA GARISAN
                    for i in range(num_points):
                        p1_coords = [df.iloc[i]['lat'], df.iloc[i]['lon']]
                        p2_coords = [df.iloc[(i + 1) % num_points]['lat'], df.iloc[(i + 1) % num_points]['lon']]
                        
                        # Garisan Sempadan
                        folium.PolyLine(locations=[p1_coords, p2_coords], color="white", weight=st_line_weight, opacity=0.8).add_to(m)
                        ...
                        folium.map.Marker(
                            ...
                            icon=folium.DivIcon(
                                icon_anchor=(30, 0),
                                html=f"""<div style="font-family: sans-serif; color: white; font-weight: bold; font-size: {st_text_size}pt; text-shadow: 1px 1px 2px black; text-align: center; white-space: nowrap;">
                                        {line_data[i]['bearing']}<br>{line_data[i]['dist']:.2f}m</div>"""
                            )
                        ).add_to(m)

                    # C. MARKER STESEN & NOMBOR
                    for _, row in df.iterrows():
                        folium.CircleMarker(
                            location=[row['lat'], row['lon']],
                            radius=st_marker_size/4, color="white", weight=2, fill=True, fill_color="red", fill_opacity=1,
                        ...
                        folium.map.Marker(
                            ...
                            icon=folium.DivIcon(
                                icon_anchor=(-10, 10),
                                html=f"""<div style="font-family: sans-serif; color: yellow; font-weight: bold; font-size: 14pt; text-shadow: 2px 2px #000;">{int(row['STN'])}</div>"""
                            )
                        ).add_to(m)
                ...
                with st.sidebar:
                    ...
                    export_points_df = df[['STN', 'E', 'N']].copy()
                    gdf_points = gpd.GeoDataFrame(
                        export_points_df, 
                        geometry=gpd.points_from_xy(export_points_df.E, export_points_df.N),
                        crs=f"EPSG:{epsg_code}"
                    )

                    poly_attr = {'Luas_m2': round(luas, 2), 'Parameter_m': round(perimeter, 2)}
                    ...
                    st.download_button(
                        ...
                        file_name="data_survey_puo.geojson",
                    ...
                    if st.button("🔑 Tukar Kata Laluan", use_container_width=True):
                        st.info("Sila hubungi admin untuk menukar kata laluan.")
